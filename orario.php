...
function get_data() {
  $list = array();
  $result = dbquery("SELECT doc_id, doc_nome FROM docenti ORDER BY doc_nome");
  while ($data = dbarray($result))
  {
    ...
    $list[] = array($id,$data['doc_nome'],$orario); // lista completa dell'orario
  }
  return $list;
}
...
function draw_dform($id,$nome)
{
  echo '<form name="frm" method="get" action="'.$_SERVER['PHP_SELF'].'" onsubmit="return check_doc(this)"><table align="center">'."\n";
  echo '<tr><td class="tbl_form">&nbsp;NOME&nbsp;</td><td><input type="text" name="nome" value="'.$nome.'" class="textbox" size="20" maxlength="20" ></td></tr>'."\n";
  echo '</td></tr></table><br>'."\n";
...
if(isset($_GET['cmd']))
{
  switch($_GET['cmd'])
  {
    case '1':   // modifica
      $id = isNum($_GET['id']);
      $result = dbquery("SELECT doc_id, doc_nome FROM docenti,orario WHERE doc_id = ora_docente AND ora_id = $id");
      $data = dbarray($result);
      $id_nome = $data['doc_id'];
      $nome = $data['doc_nome'];
    ...
    case '5':   // modifica
      ...
      $data = dbarray($result);
      draw_dform($data['doc_id'],$data['doc_nome']);
      break;
    case '6':  //conferma modifica
      if($_GET['id'])
      {
        $result = dbquery('UPDATE docenti SET doc_nome = "'.isText($_GET['nome']).'" WHERE doc_id='.isNum($_GET['id']));
        if($result && dbrows($result,"update"))
          message('Dati modificati con successo');
      }
      else
      {
        $result = dbquery('INSERT INTO docenti VALUES ( DEFAULT, "'.isText($_GET['nome']).'")');
        if($result && dbrows($result,"insert"))
          message('Dati aggiunti con successo');
      }
      break;
